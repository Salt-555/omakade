...
QVariant DolphinGameModel::valueForRole(const Game& game, int role) const {
  ...
  case GameRoles::PlaytimeProvenance:
    return PlaySessionStore::provenance(m_playSessions, game.dolphin.path, -1);
  ...
  case GameRoles::Recent:
    return PlaySessionStore::displayedLastPlayed(m_playSessions, game.dolphin.path, 0) > 0;
  case GameRoles::LastPlayed:
    return PlaySessionStore::displayedLastPlayed(m_playSessions, game.dolphin.path, 0);
